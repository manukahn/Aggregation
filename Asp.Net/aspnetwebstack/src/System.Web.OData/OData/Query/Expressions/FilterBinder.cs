...
namespace System.Web.OData.Query.Expressions
{
    ...
    internal class FilterBinder
    {
        ...
        public static LambdaExpression Bind(ExpressionClause expressionClause, Type expressionType, IEdmModel model,
            IAssembliesResolver assembliesResolver, ODataQuerySettings querySettings, ParameterExpression entityParam = null)
        {
            LambdaExpression result;
            if (expressionClause == null)
            {
                throw Error.ArgumentNull("expressionClause");
            }
            if (expressionType == null)
            {
                throw Error.ArgumentNull("expressionType");
            }
            if (model == null)
            {
                throw Error.ArgumentNull("model");
            ...
            if (entityParam == null)
            {
                result = binder.BindExpression(expressionClause.Expression, expressionClause.RangeVariable, expressionType);
            }
            else
            {
                result = binder.BindExpression(expressionClause.Expression, entityParam);
            }
        ...
        private LambdaExpression BindExpression(SingleValueNode expression, ParameterExpression filterParameter)
        {
            _lambdaParameters = new Dictionary<string, ParameterExpression>();
            _lambdaParameters.Add(filterParameter.Name, filterParameter);
            Expression body = Bind(expression);
            return Expression.Lambda(body, filterParameter);
        }
